#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PDF PROFESIONAL de la documentación de la conexión con la tienda en línea (Magento), 24-ago-2026.
# Mismo formato de documento controlado que el de MCP-SAP: portada con código, versión, clasificación
# y control de cambios. Fuente única: docs/CUADERNO-CASOS-REALES.md
# Uso: python3 gen_doc_tienda_pdf.py
...
contenido = md_a_html(open("docs/CUADERNO-CASOS-REALES.md").read())

cuerpo = """
<div class="portada">
  <img src="LOGO_DATAURI" style="width:180px;max-width:55%%;margin-bottom:16px"><p class="kicker">GRUPO ARDISA · CUADERNO DE TRABAJO</p>
  <h1 class="titulo">Cuaderno de Casos Reales</h1>
  <p class="sub">Cinco fallos de producción, con los comandos para diagnosticarlos tú misma</p>
  <table class="ficha">
    <tr><td>Código del documento</td><td>CUAD-BOT-001</td></tr>
    <tr><td>Versión</td><td>1.0</td></tr>
    <tr><td>Fecha</td><td>%s</td></tr>
    <tr><td>Para</td><td>Deicy Milena Jejen — Desarrolladora del Bot WhatsApp</td></tr>
    <tr><td>Cómo se usa</td><td>Teclear la sección 🔍 ANTES de leer la sección ✅</td></tr>
  ...
  <table class="ficha cambios">
    ...
    <tr><td>1.0</td><td>%s</td><td>Cinco casos de producción del 24 al 26 de agosto: el precio escondido,
        la alerta que se detectaba a sí misma, preventa clasificada como PQRS, la migración a la tienda
        como fuente de datos y la caché de instrucciones. Con ejercicios y examen final</td></tr>
  </table>
</div>
%s
<div class="cierrefinal">
  <p><b>Fin del documento.</b> Este documento se genera desde el repositorio del proyecto
  (<code>docs/CUADERNO-CASOS-REALES.md</code> + <code>gen_doc_tienda_pdf.py</code>): toda actualización queda
...
css = css + _marca.CSS_APA
doc = "<!DOCTYPE html><html><head><meta charset='utf-8'><style>%s</style></head><body>%s</body></html>" % (css, cuerpo)
open("/tmp/cuaderno_casos.html", "w").write(doc.replace("LOGO_DATAURI", _marca.logo_datauri()))
subprocess.run(["wkhtmltopdf", "-q", "--enable-local-file-access",
                "/tmp/cuaderno_casos.html", "docs/Cuaderno-Casos-Reales.pdf"], check=True)
print("OK -> docs/Cuaderno-Casos-Reales.pdf")
