...
# y control de cambios. Fuente única: docs/CONEXION-TIENDA-MAGENTO.md
...
contenido = md_a_html(open("docs/CONEXION-TIENDA-MAGENTO.md").read())
...
<div class="portada">
  ...
  <h1 class="titulo">Conexión del Bot de WhatsApp<br>con la Tienda en Línea (Magento)</h1>
  <p class="sub">Catálogo, enlaces al producto y precios — arquitectura, seguridad y verificación</p>
  <table class="ficha">
    <tr><td>Código del documento</td><td>DOC-BOT-WEB-001</td></tr>
  ...
  <table class="ficha cambios">
    ...
    <tr><td>1.0</td><td>%s</td><td>Versión inicial: GraphQL público para búsqueda (OpenSearch), REST autenticado
        con integración de solo lectura, regla del 1%% para el enlace, alternativas publicadas cuando no hay ficha,
        y los cuatro obstáculos de autenticación resueltos</td></tr>
  </table>
</div>
%s
<div class="cierrefinal">
  <p><b>Fin del documento.</b> Este documento se genera desde el repositorio del proyecto
  (<code>docs/CONEXION-TIENDA-MAGENTO.md</code> + <code>gen_doc_tienda_pdf.py</code>): toda actualización queda
...
open("/tmp/doc_tienda_pdf.html", "w").write(doc.replace("LOGO_DATAURI", _marca.logo_datauri()))
subprocess.run(["wkhtmltopdf", "-q", "--enable-local-file-access",
                "/tmp/doc_tienda_pdf.html", "docs/Doc-Conexion-Tienda-Magento.pdf"], check=True)
print("OK -> docs/Doc-Conexion-Tienda-Magento.pdf")
